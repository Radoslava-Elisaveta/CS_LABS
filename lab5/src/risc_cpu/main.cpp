...
int sc_main(int, char *[])
{
  ...
  sc_trace_file* tf = sc_create_vcd_trace_file("signals");

  //((vcd_trace_file*)tf)->sc_set_vcd_time_unit(0);
  sc_trace(tf, clk, "CLK");
  //sc_trace(tf, icache_valid, "ICACHE_VALID");
  //sc_trace(tf, ram_cs, "RAM_CS");
  //sc_trace(tf, ram_we, "RAM_WE");
  //sc_trace(tf, addr, "Address");
  //sc_trace(tf, ram_datain, "RAM_DATAIN");
  //sc_trace(tf, ram_dataout, "RAM_DATAOUT");
  //sc_trace(tf, bios_valid, "BIOS_VALID");
  //sc_trace(tf, icache_din, "ICACHE_DIN");
  //sc_trace(tf, icache_validin, "ICACHE_VALIDIN");
  //sc_trace(tf, icache_stall, "ICACHE_STALL");
  //sc_trace(tf, paging_dout, "PAGING_DOUT");
  //sc_trace(tf, paging_csout, "PAGING_CSOUT");
  //sc_trace(tf, paging_weout, "PAGING_WEOUT");
  //sc_trace(tf, branch_target_address, "BRANCH_TARGET_ADDRESS");
  //sc_trace(tf, next_pc, "NEXT_PC");
  //sc_trace(tf, next_pc, "NEXT_PC");
  //sc_trace(tf, branch_valid, "BRANCH_VALID");
  //sc_trace(tf, stall_fetch, "STALL_FETCH");
  //sc_trace(tf, pred_fetch, "PRED_FETCH");
  //sc_trace(tf, instruction, "INSTRUCTION");
  //sc_trace(tf, instruction_valid, "INSTRUCTION_VALID");
  //sc_trace(tf, program_counter, "PROGRAM_COUNTER");
  //sc_trace(tf, branch_clear, "BRANCH_CLEAR");
  //sc_trace(tf, pred_fetch_valid, "PRED_FETCH_VALID");
  //sc_trace(tf, reset, "RESET");
  //sc_trace(tf, pred_branch_address, "PRED_BRANCH_ADDRESS");
  //sc_trace(tf, pred_branch_valid, "PRED_BRANCH_VALID");
  //sc_trace(tf, pred_tellid, "PRED_TELLID");
  //sc_trace(tf, pred_instruction, "PRED_INSTRUCTION");
  //sc_trace(tf, pred_inst_valid, "PRED_INST_VALID");
  //sc_trace(tf, pred_inst_pc, "PRED_INST_PC");
  //sc_trace(tf, pred_on, "PRED_ON");
  //sc_trace(tf, branch_instruction_address, "BR_INSTRUCTION_ADDRESS");
  //sc_trace(tf, dram_dataout, "DRAM_DATAOUT");
  //sc_trace(tf, dram_rd_valid, "DRAM_RD_VALID");
  //sc_trace(tf, dram_write_src, "DRAM_WRITE_SRC");
  //sc_trace(tf, mem_access, "MEM_ACCESS");
  //sc_trace(tf, mem_address, "MEM_ADDRESS");
  ...
  //sc_trace(tf, forward_A, "FORWARD_A");
  //sc_trace(tf, forward_B, "FORWARD_B");
  //sc_trace(tf, decode_valid, "DECODE_VALID");
  //sc_trace(tf, float_valid, "FLOAT_VALID");
  //sc_trace(tf, mmx_valid, "MMX_VALID");
  //sc_trace(tf, pid_valid, "PID_VALID");
  //sc_trace(tf, pid_data, "PID_DATA");
  //sc_trace(tf, mmic_datain, "MMIC_DATAIN");
  //sc_trace(tf, mmic_statein, "MMIC_STATEIN");
  //sc_trace(tf, mmic_cs, "MMIC_CS");
  //sc_trace(tf, mmic_we, "MMIC_WE");
  //sc_trace(tf, mmic_addr, "MMIC_ADDR");
  //sc_trace(tf, mmic_dest, "MMIC_DEST");
  //sc_trace(tf, mmic_destout, "MMIC_DESTOUT");
  //sc_trace(tf, mmic_dataout, "MMIC_DATAOUT");
  //sc_trace(tf, mmic_out_valid, "MMIC_OUT_VALID");
  //sc_trace(tf, mmic_stateout, "MMIC_STATEOUT");
  ...
  /*sc_trace(tf, out_valid, "OUTPUT_VALID");
  sc_trace(tf, destout, "DESTOUT");
  sc_trace(tf, fdout, "FDOUT");
  sc_trace(tf, fout_valid, "FOUT_VALID");
  ...
  sc_trace(tf, intack_cpu, "INTACK_CPU");
  sc_trace(tf, dsp_in1, "DPS_IN1");
  sc_trace(tf, dsp_out1, "DSP_OUT1");
  sc_trace(tf, dsp_data_valid, "DSP_DATA_VALID");
  sc_trace(tf, dsp_input_valid, "DSP_INPUT_VALID");
  sc_trace(tf, dsp_data_requested, "DSP_DATA_REQUESTED");*/
  sc_start();
